...
\address{\#2605-170 Bayview Avenue, Toronto, Ontario, Canada, M4M 2X2}
...
A radically candid, creative software engineer with a strong cross-technology skill-set. Looking for opportunities to leverage my existing skills in Site-Reliability Engineering, Performance, and related Dev/Ops work. The goal is always to ship fantastic products that make people's lives better.
...
Network Security, Internet of Things, Privacy, Capacity Plannings.
...
\emph{Programming Languages}: Golang, C.

\emph{Scripting:} Python, Perl, Ruby, Lua, Various Javascript Toolkits.

\emph{Data Management}: PostgreSQL, MySQL, Redshift/HADOOP.

\emph{Toolkits}: Hashicorp stack (Terraform/Nomad/Vault), Apache Hadoop stack, Rails/Sinatra, Node.js, AWS/Heroku/Digital Ocean.

\emph{Tools}: NeoVIM, Git, Visual Studio 2003-2008.

\emph{Software Engineering Management}: Team Topologies and  Agile/XP Development, Design Patterns.
...
\begin{position}
\begin{itemize}
\item Prototyped a data center migration off AWS and into a platform that I am NDA blocked from revealing. We determined this is a very bad idea.
\item Built a PII preserving datalake on AWS Athena that ingests from a variety of data centers. The ETL pipeline was initially was built using hashicorp tools, but later migrated to AWS Batch for simplicity and cost savings.
\item Built and staffed out a BI team on top of the datalake to provide business services via Looker for curated BI and Metabase for raw data access.
\item Supported senior leadership directly to fullfill organization goals as a senior member of the infrastructure team.
\end{itemize}
\end{position}
